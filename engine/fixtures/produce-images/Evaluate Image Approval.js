...
recs.forEach((r, i) => {
  ...
  if (f['Regenerează Imagine'] === true) {
    ...
    const planned = CONS.plan({ f: f, opts: regenOpts, bible: regenBible, prompt: prompt, prevId: prevId, userRefId: '', isFirstScene: false, afterRefusal: rejected, strict: false, strictNotes: '', continuity: String(regenOpts.category || '') === 'cinematic', prevF: i > 0 ? (recs[i - 1].json.fields || {}) : null });
