...
// ---------------------------------------------------------------------------
const planned = CONS.plan({ f: f, opts: opts, bible: bible, prompt: prompt, prevId: prevId, userRefId: userRefId, isFirstScene: isFirstScene, afterRefusal: afterRefusal, strict: strict, strictNotes: strictNotes, continuity: continuity, prevF: prevF });
...
return [{ json: { sceneId: $json.id, requestBody: body, refs: planned.refs, used: planned.used, usedReference: planned.refs.length ? planned.refs[0].id : null, castUsed: planned.used.cast, userRefApplied: planned.used.user, strict: strict, rawPrompt: prompt, promptSimilarityToPrev: Number(promptSim.toFixed(2)), ...(continuity ? { locTags: CONS.tagged(f, 'loc:') } : {}) } }];