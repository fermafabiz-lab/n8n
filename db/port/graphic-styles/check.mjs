#!/usr/bin/env node
//
// check.mjs — run the graphic-styles node bodies against fixtures, with no n8n
// and no network.
//
//     node db/port/graphic-styles/check.mjs
//
...
console.log('Build Plan Prompt');
{
  ...
  ok('offers the style choice when nothing is picked', r[0].json.payload.messages[0].content.includes('FIRST choose'));
  ...
  ok('a producer pick is passed on, no style menu', p[0].json.picked === 'cinematic' && !p[0].json.payload.messages[0].content.includes('FIRST choose'));
  for (const [label, film] of [
    ['kids story', { ...FILM, category: 'kids' }],
    ['cinematic category', { ...FILM, category: 'cinematic' }],
    ['explicit classic', { ...FILM, graphic_style: 'classic' }],
...
});
{
  const { out, logs } = parse({
    style: 'reportage', why: 'An investigation of supply.',
    items: [
...
}).out[0].json.body;
{
  ...
  for (const opts of [{}, { category: 'story', motionPack: 'punch', captionColor: '#ff0' }, { graphicStyle: 'classic', category: 'kids' }]) {
...
console.log('Normalize Webhook Input');
{
  ...
  ok('only the graphic_style lines were added', added.every((l) => /graphic|^\s*\/\/|^\s*$/.test(l)), added.join('\n'));
  ok('the original is contained whole', src.replace(/\n    \/\/ Which graphics ride[\s\S]*?\{ graphicStyle: s \} : \{\}; \}\)\(\)\),\n/, '\n') === orig.trimEnd());
}

console.log(failures ? `\n${failures} FAILED` : '\nall graphic-styles checks passed');
