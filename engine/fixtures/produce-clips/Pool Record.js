...
//
// Six edges arrive here, and they mean different things. They are told apart by
...
if (from === 'Pool Submitted?') {
  ...
  // Submit Video failed after its own quick retries (Submit Cooldown Guard,
  // 2026-09-24). The scene goes back to the HEAD of the queue on the same
  // account, and that account rests for a minute; the other accounts' clips
  // keep being polled meanwhile. This used to be a 60-second Wait inside the
  // tick, looped up to twenty times, which froze every poll of the film.
  const g = $input.first().json || {};
  const prev = inflight.find(function (x) { return x.id === sceneId; });
  const q = queue.find(function (x) { return x.id === sceneId; });
  const stolen = (pool.stolen || {})[sceneId] || null;
  // The still this attempt was made from. A resubmit after the refusal ladder
  // carries a NEW still that Sort Scenes For Video (Pool Tick's source for the
  // row) does not know about, so it is kept on the pool and put back on the
  // row when the scene is picked again.
  let img = '';
  try { img = String(((scene && scene.videoRequest) || {}).startImage || ((scene && scene.fields) || {})['Image Media ID'] || ''); } catch (e) { img = ''; }
  let owner = '';
  const hx = img.match(/-email:([0-9a-f]+)-/i);
  ...
  if (g.giveUp) {
    ...
    pool.done = done;
    console.log('POOL gave up submitting ' + sceneId + ' on ' + (account || '(primary)') + ' — the next pass will find it without a clip');
  } else {
    const REST_MS = 60 * 1000;
    pool.cooldownUntil = pool.cooldownUntil || {};
    pool.cooldownUntil[account] = Date.now() + REST_MS;
    if (img) {
      pool.freshImage = pool.freshImage || {};
      pool.freshImage[sceneId] = img;
    }
    pool.queue.unshift({ id: sceneId, account: account });
    console.log('POOL submit failed ' + sceneId + ' on ' + (account || '(primary)') + ' (' + (g.cooldown || '?') + '): account rests 60s, scene back at the head of the queue (' + pool.inflight.length + ' in flight, still polled)');
  }
