 ...
 *
 * MIRRORED in `platform/lib/provenance-glyphs.ts`, exactly like the labels and
 * the layout next door — the site's watermark preview draws the same mark the
 * film does, or it is not a preview. `npm run check:watermark` here and
 * `npm run check:footage` there pin that every origin has one.
 *
