...
// ---------------------------------------------------------------------------
const planned = CONS.plan({ f: f, opts: opts, bible: bible, prompt: prompt, prevId: prevId, userRefId: userRefId, isFirstScene: isFirstScene, afterRefusal: rejectedBefore, strict: false, strictNotes: '', continuity: continuity, prevF: idx > 0 ? (all[idx - 1].fields || {}) : null });
// count: 1 is mandatory (the API defaults to four). captchaRetry: 5, like
// every batch Flow body since 2026-09-23 (db/port/captcha-retry/): a 403
// PUBLIC_ERROR_UNUSUAL_ACTIVITY is a captcha token Google rejected, not an
// account flag, and at 1 attempt most regenerations failed — measured
// 2026-09-25 20:17-20:24, 17% of captcha attempts accepted, so one try is a
// coin flip weighted against you and five pass about nine times in ten.
const body = CONS.apply({ email: 'fermafabiz@gmail.com', model: MODEL, prompt: prompt, aspectRatio: aspect, count: 1, captchaRetry: 5 }, planned);
