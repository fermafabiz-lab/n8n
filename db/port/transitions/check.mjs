...
console.log('Build Plan Prompt');
{
  ...
  ok('marks the cold open', r[0].json.payload.messages[1].content.includes('SCENE 1 (cold open'));
  ...
  for (const [label, film] of [
    ['kids story with a picked transition', { ...FILM, category: 'kids', transition_style: 'crossfade' }],
    ['cinematic category with a picked transition', { ...FILM, category: 'cinematic', transition_style: 'none' }],
  ...
  const kids = runNode('paste/Build Plan Prompt.js', { json: { ...FILM, category: 'kids' } }).out;
  ok('a kids film is asked for its transition only', kids.length === 1 && kids[0].json.graphics === false
    && kids[0].json.payload.messages[0].content.includes('TRANSITION') && !kids[0].json.payload.messages[0].content.includes('GRAPHICS'));
...
});
{
  ...
  ok('refusals are logged with reasons', logs.some((l) => l.includes('quote not in the narration') && l.includes('title not spoken')), logs.join('\n'));
...
{
  const kAsked = runNode('paste/Build Plan Prompt.js', { json: { ...FILM, category: 'kids' } }).out[0].json;
  const k = parse({ transition: 'crossfade', why: 'gentle', items: [{ kind: 'person', scene: 2, quote: 'Augustus appointed', title: 'Augustus' }] }, kAsked).out[0].json.plan;
  ok('a kids plan: classic, the transition, no items', k.style === 'classic' && k.transition === 'crossfade' && k.items.length === 0, JSON.stringify(k));
  ok('a kids plan with no valid transition writes nothing', parse({ transition: 'wipe' }, kAsked).out.length === 0);
...
console.log('Normalize Webhook Input');
{
  ...
  ok('only the transition_style lines were added', added.every((l) => /transition|^\s*\/\/|^\s*$/.test(l)), added.join('\n'));
  ok('the original is contained whole', src.replace(/\n    \/\/ How the picture hands over[\s\S]*?\{ transitionStyle: s \} : \{\}; \}\)\(\)\),\n/, '\n') === orig.trimEnd());
}

console.log(failures ? `\n${failures} FAILED` : '\nall graphic-styles + transitions checks passed');
