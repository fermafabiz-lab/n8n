...
import {FLASH_IN, FLASH_LEAD, LightLeak, flashEnvelope, flashSweep} from './LightLeak';
 ...
 *
 * It arrives on the SAME light leak the chapter card uses, and for the same
 * reason. It used to fade its opacity up over 0.4s while sliding 24px — a
 * dissolve, which announces a graphic, and a linear one at that. The end
 * screen replaces the picture outright, which is the most total cut in the
 * film; hiding it inside a flash is what makes it read as an edit rather than
 * as an overlay appearing. Whole at the peak, nothing fading.
 *
 * The caller leads the Sequence by FLASH_LEAD so the peak lands on the frame
 * the footage gives way.
 */
export const OutroCard: React.FC<{
	...
	const t = frame / fps;
	const inFlash = flashEnvelope(t / FLASH_IN);
	// The subscribe pill still pops, but a beat AFTER the card has landed rather
	// than during its arrival — it is an accent on something already on screen,
	// not part of the entrance.
	const buttonScale = spring({
		frame: frame - Math.round((FLASH_LEAD + 0.35) * fps),
		fps,
		config: {damping: 14, stiffness: 160},
	});
	const pulse = 1 + 0.03 * Math.sin(t * Math.PI * 2);
	...
	return (
		<AbsoluteFill>
			...
			{/* The flare itself, over whatever is underneath — the tail of the
			    footage before the peak, the end screen after it. */}
			<LightLeak amount={inFlash} sweep={flashSweep(t / FLASH_IN)} />
		</AbsoluteFill>
	);
};
