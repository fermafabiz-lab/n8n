...
const nodes = (baseX, baseY) => [
	...
	{
		parameters: {
			// One card per variant, and each carries ONLY its own motif's fields.
			// The single example this used to hold put a route's `stops` beside a
			// schedule's `rows` and justified them with one `sources` map keyed by
			// path — and the first real film came back with a route card whose only
			// source sat under `rows[0].value`. Every string in it was true; the
			// card was dropped for having no source. A shape that cannot express
			// the mistake is worth more than an instruction not to make it.
							...
							marks: [
								{
									at: '1941',
									label: 'as a dealer',
									source: {kind: 'quote', sceneIndex: 2, from: 'exact words from that scene'},
								},
							],
							note: '34 years',
							...
							rows: [
								{
									label: 'Feribot',
									value: '05:20',
									source: {kind: 'quote', sceneIndex: 6, from: 'exact words from that scene'},
								},
							],
